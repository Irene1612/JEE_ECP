...
<body>
	...
	<form action="/VotacionesApp/jsp/procesarVoto" method="post">
		Seleccione una puntuación para votar este tema: <select
			name="valoracion">
			<c:forEach var="iterador" begin="0"
				end="${votarTemaView.valorMaximoVotacion}">
				<option value="${iterador}">${iterador}</option>
			</c:forEach>
		</select><br> Seleccione el nivel de estudios: <select
			name="nivelEstudios">
			<c:forEach var="iterador" begin="0"
				end="${votarTemaView.cantidadNiveles}">
				<option value="${iterador}">${votarTemaView.nivelesEstudiosString[iterador]}</option>
			</c:forEach>
		</select> <input type="hidden" name="tema" value="${votarTemaView.id}">
