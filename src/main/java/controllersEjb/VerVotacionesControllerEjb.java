...
public class VerVotacionesControllerEjb extends TemaControllerEjb implements
	...
	public NivelEstudioVotos[] calcularMediaVotos(List<Voto> votos) {
		NivelEstudioVotos[] nivelEstudioVotos = new NivelEstudioVotos[NivelEstudios
				.values().length];
		...
		if (votos.size() != 0) {
			...
			for (int i = 0; i < votos.size(); i++) {
				...
				case SIN_ESTUDIOS:
					...
					if (valoracionAcumuladaEstudio[NivelEstudios.SIN_ESTUDIOS
							.ordinal()] == null) {
						valoracionAcumuladaEstudio[NivelEstudios.SIN_ESTUDIOS
								.ordinal()] = 0.0;
					}
					valoracionAcumuladaEstudio[NivelEstudios.SIN_ESTUDIOS
							.ordinal()] += votos.get(i).getValoracion();
					LogManager
							.getLogger(VerVotacionesControllerEjb.class)
							.debug("Hay"
									+ numeroVotosEstudio[NivelEstudios.SIN_ESTUDIOS
											.ordinal()]
									+ "voto de SIN ESTUDIOS");
					LogManager
							.getLogger(VerVotacionesControllerEjb.class)
							.debug("Valoracion acumulada: "
									+ valoracionAcumuladaEstudio[NivelEstudios.SIN_ESTUDIOS
											.ordinal()] + " SIN ESTUDIOS");
					break;
				case ESO:
					...
					valoracionAcumuladaEstudio[NivelEstudios.ESO.ordinal()] += votos
							.get(i).getValoracion();
					LogManager.getLogger(VerVotacionesControllerEjb.class).debug(
							"Hay"
									+ numeroVotosEstudio[NivelEstudios.ESO
											.ordinal()] + "voto de ESO");
					LogManager
							.getLogger(VerVotacionesControllerEjb.class)
							.debug("Valoracion acumulada: "
									+ valoracionAcumuladaEstudio[NivelEstudios.ESO
											.ordinal()] + " ESO");
					break;
				case BACHILLERATO_FP:
					numeroVotosEstudio[NivelEstudios.BACHILLERATO_FP.ordinal()]++;
					if (valoracionAcumuladaEstudio[NivelEstudios.BACHILLERATO_FP
							.ordinal()] == null) {
						valoracionAcumuladaEstudio[NivelEstudios.BACHILLERATO_FP
								.ordinal()] = 0.0;
					}
					valoracionAcumuladaEstudio[NivelEstudios.BACHILLERATO_FP
							.ordinal()] += votos.get(i).getValoracion();
					LogManager
							.getLogger(VerVotacionesControllerEjb.class)
							.debug("Hay"
									+ numeroVotosEstudio[NivelEstudios.BACHILLERATO_FP
											.ordinal()]
					...
					LogManager
							...
							.debug("Valoracion acumulada: "
									+ valoracionAcumuladaEstudio[NivelEstudios.BACHILLERATO_FP
											.ordinal()] + " BACHILLERATO_FP");
					break;
				case UNIVERSITARIO:
					numeroVotosEstudio[NivelEstudios.UNIVERSITARIO.ordinal()]++;
					if (valoracionAcumuladaEstudio[NivelEstudios.UNIVERSITARIO
							.ordinal()] == null) {
						valoracionAcumuladaEstudio[NivelEstudios.UNIVERSITARIO
								.ordinal()] = 0.0;
					}
					valoracionAcumuladaEstudio[NivelEstudios.UNIVERSITARIO
							.ordinal()] += votos.get(i).getValoracion();
					LogManager
							.getLogger(VerVotacionesControllerEjb.class)
							.debug("Hay"
									+ numeroVotosEstudio[NivelEstudios.UNIVERSITARIO
											.ordinal()]
			...
			for (int i = 0; i < NivelEstudios.values().length; i++) {
				nivelEstudioVotos[i] = new NivelEstudioVotos(
						NivelEstudios.values()[i].toString());
				LogManager.getLogger(VerVotacionesControllerEjb.class).debug(
						"Valoracion acumulada final de "
								+ NivelEstudios.values()[i] + ": "
								+ valoracionAcumuladaEstudio[i]);
				if (valoracionAcumuladaEstudio[i] != null) {
					Double media = nivelEstudioVotos[i].calcularMedia(valoracionAcumuladaEstudio[i], numeroVotosEstudio[i]);
	...
	private void inicializarArrays(int[] numeroVotosEstudio,
			Double[] valoracionAcumuladaEstudio) {
		for (int i = 0; i < NivelEstudios.values().length; i++) {
